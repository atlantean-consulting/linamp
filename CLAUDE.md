...
- `linamp/stations.py` — `Station` dataclass (name, url, genre) + `Folder` dataclass (name, stations, channel_url). `channel_url` is optional — when set, the folder is a "channel folder" (imported YouTube channel/playlist) and `is_channel` property returns True. `load_library()` reads `~/.config/linamp/stations.json` with auto-migration from old flat format. `save_library()` writes folder structure (including `channel_url` when present). `all_stations()` helper flattens folders but **excludes channel folders** — channel videos only play when explicitly selected, not as part of the radio playlist rotation. `_migrate_flat_list()` auto-categorizes stations into Radio/YouTube folders based on URL. Legacy `load_stations()`/`save_stations()`/`STATIONS` aliases kept for backwards compat.
...
- `linamp/widgets/station_list.py` — Left pane browser. Uses Textual `Tree` widget (not ListView) with folder nodes (📁) and station leaves (📻 for radio, 🎵 for YouTube). Full CRUD: add/delete/rename/edit stations and folders, move stations within folders. Inline `Input` widgets for editing. YouTube URL auto-detection: fetches title via yt-dlp in background thread (`asyncio.to_thread`), auto-fills name and genre. Add flow is URL-first to enable auto-detection. Delete requires double-press confirmation. **YouTube channel import** (`c`): prompts for a channel/playlist URL, fetches up to 25 videos via `yt-dlp --flat-playlist` in a background thread, creates a channel folder (with `channel_url` set). Channel folders are browse-only — videos don't appear in the radio playlist. `m` loads 25 more videos from the channel (paginated via `--playlist-start`/`--playlist-end`). `w` saves the selected channel video to the regular YouTube folder so it appears in the playlist. Pagination state (`_channel_meta` dict) tracks offset and exhaustion per channel folder; initialized on demand so `m` works even on channels loaded from disk.
...
- **Format**: `{"folders": [{"name": "Radio", "stations": [{"name": "...", "url": "...", "genre": "..."}]}]}`. Channel folders include `"channel_url": "https://..."` — this marks them as browse-only (excluded from `flat_stations`/radio playlist).
...
| c | Import YouTube channel/playlist (prompts for URL, fetches 25 videos) |
| m | Load more videos from channel folder (next 25) |
| w | Save selected channel video to YouTube folder (adds to playlist) |
